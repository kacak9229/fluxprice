...
export default function DemandPricingDashboard() {
  ...
  // Scale: 4% width per 1% adjustment. Max adjustment 12% -> 48% width (close to 50%).
  const widthPercentage = Math.abs(priceAdjustment) * 4;
  
  const barLeft = isHighDemand ? "50%" : `${50 - widthPercentage}%`;

  return (
    <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-2xl max-w-[400px] mx-auto font-sans relative overflow-hidden transform hover:scale-[1.02] transition-transform duration-500 group">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-gray-900">Demand Tracker</h3>
            <p className="text-[10px] text-gray-500">Real-time Analysis</p>
          </div>
        </div>
      ...
      <div className="space-y-8 relative z-10">
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            >
              {isHighDemand ? (
                <>
                  <TrendingUp className="w-5 h-5" /> High Demand
                </>
              ) : (
                <>
                  <TrendingDown className="w-5 h-5" /> Low Demand
                </>
              )}
            </motion.div>
          </div>
          ...
          <div className="absolute inset-0 flex items-center justify-between px-6 pointer-events-none z-20">
            <span
              className={`text-[10px] font-bold transition-colors duration-300 ${
                !isHighDemand ? "text-orange-600" : "text-gray-300"
              }`}
            >
              LOW DEMAND
            </span>
            <span
              className={`text-[10px] font-bold transition-colors duration-300 ${
                isHighDemand ? "text-blue-600" : "text-gray-300"
              }`}
            >
              HIGH DEMAND
            </span>
        ...
        <motion.div
          className="flex items-center justify-center gap-2 text-xs font-medium text-emerald-700 bg-emerald-50/80 backdrop-blur-sm py-3 rounded-xl border border-emerald-100/50 shadow-sm"
